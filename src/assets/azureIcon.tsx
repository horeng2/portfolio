...
export function AzureIcon({ className, ...rest }: AzureIconProps) {
  return (
    <div className={`group relative inline-block h-6 w-6 ${className ?? ""}`}>
      {/* 1) 회색 단색 버전 (기본 상태) */}
      <svg
        viewBox="0 0 96 96"
        aria-hidden="true"
        fill="currentColor"
        className="absolute inset-0 text-slate-500 transition-opacity group-hover:opacity-0"
        // props는 여기 안 넘김 (className 깨지지 않게)
      >
      ...
      {/* 2) 컬러 그라데이션 버전 (hover 상태) */}
      <svg
        viewBox="0 0 96 96"
        aria-hidden="true"
        className="absolute inset-0 opacity-0 transition-opacity group-hover:opacity-100"
        {...rest} // 여긴 width/height 같은 것만 먹도록 둠
      >
        <defs>
          <linearGradient
            id="g1"
